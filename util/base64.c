...
int base64_decode(uint8_t *out, const char *in_str, int out_size)
{
    uint8_t *dst = out;
                              ...
                              const char *encoded_ref)
{
    ...
    if (!base64_encode(encoded, MAX_ENCODED_SIZE, data, data_size)) {
    ...
    if ((data2_size = base64_decode(data2, encoded, max_data2_size)) != data_size) {
        printf("Failed: cannot decode the encoded string\n"
               "Encoded:\n%s\n", encoded);
        return 1;
    }
    if ((data2_size = base64_decode(data2, encoded, data_size)) != data_size) {
    ...
    if (base64_decode(NULL, encoded, 0) != 0) {
...
int main(int argc, char ** argv)
{
    ...
    if (argc>1 && !strcmp(argv[1], "-t")) {
        ...
        for(i=0; i<10000; i++){
            START_TIMER
            base64_encode(out, sizeof(out), in, sizeof(in));
            STOP_TIMER("encode")
        }
        for(i=0; i<10000; i++){
            START_TIMER
            base64_decode(in, out, sizeof(in));
            STOP_TIMER("decode")
        }

        for(i=0; i<10000; i++){
            START_TIMER
            base64_decode(NULL, out, 0);
